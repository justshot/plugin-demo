...
<head>
    <meta charset="utf-8">
    <title>JSDoc: Source: rtc/peerconnection.js</title>
...
<div id="main">
    ...
    <h1 class="page-title">Source: rtc/peerconnection.js</h1>
...
PeerConnection.prototype._createAnalyser = (stream, audioContext) => {
...
PeerConnection.prototype._startPollingVolume = function() {
  ...
  const inputAnalyser = this._inputAnalyser = this._createAnalyser(this.stream, audioContext);
  const inputBufferLength = inputAnalyser.frequencyBinCount;
  const inputDataArray = new Uint8Array(inputBufferLength);

  const outputAnalyser = this._outputAnalyser = this._createAnalyser(this._remoteStream, audioContext);
